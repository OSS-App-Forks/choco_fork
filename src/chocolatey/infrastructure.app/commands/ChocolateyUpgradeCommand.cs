            ...
            if (!string.IsNullOrWhiteSpace(configuration.Input))
            {
                ...
                if (!configuration.Information.IsLicensedVersion)
                {
                    foreach (var argument in unparsedOptionsAndPackages.or_empty_list_if_null())
                    {
                        ...
                        {
                            this.Log().Debug("It appears you are attempting to use options that may be only available in licensed versions of Chocolatey ('{0}'). There may be ways in the open source edition to achieve what you are looking to do. Please remove the argument and consult the documentation.".format_with(arg));
                        }
                    }
                }
            }
